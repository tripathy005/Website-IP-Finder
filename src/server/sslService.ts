...
/**
 * Checks SSL certificate status and measures HTTP/HTTPS latency.
 */
export async function inspectNetworkAndSsl(domain: string, port = 443): Promise<NetworkHealth> {
  ...
  const httpPromise = new Promise<{ httpsEnabled: boolean; statusCode?: number; serverHeader?: string; redirectUrl?: string }>((resolve) => {
    try {
      ...
      req.on('error', () => {
        ...
        const httpReq = http.get(`http://${domain}`, {
          timeout: 3000,
  ...
  const [ssl, httpRes] = await Promise.all([sslPromise, httpPromise]);
